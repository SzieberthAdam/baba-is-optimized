...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        ahk_path = _thisdir / "autobaba-work-template.ahk"
        if not ahk_path.is_file():
            print(ahk_missing)
            sys.exit(20)
    ...
    txt_stat0 = txt_path.stat()
    txt_stat1 = None

    while True:

        if txt_stat0 == txt_stat1:
            time.sleep(0.25)
            txt_stat1 = txt_path.stat()
            continue
        ...
        print(f'REFRESHED {datetime.datetime.now()}')

        txt_stat0 = txt_stat1 = txt_path.stat()
